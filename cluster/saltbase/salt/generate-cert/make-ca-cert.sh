...
if [ $use_cn = "true" ]; then
    ...
    # Generate kubeapiserver cert
    ./easyrsa --subject-alt-name="${sans}" build-server-full kubeapiserver nopass > /dev/null 2>&1
    cp -p pki/issued/kubeapiserver.crt "${cert_dir}/kubeapiserver.cert" > /dev/null 2>&1
    cp -p pki/private/kubeapiserver.key "${cert_dir}/kubeapiserver.key" > /dev/null 3>&1
fi
